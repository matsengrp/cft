...
def timepoint_colors(annotations):
    ...
    def annotation_timepoints(annotation):
        return set(
            annotation.get("cluster_timepoints", annotation["timepoints"]).split(":")
        )

    timepoints = sorted(
        reduce(
            set.union, (annotation_timepoints(x) for x in annotations.values()), set()
        )
    )
    palette = [
        "#%02x%02x%02x" % c
        for c in colorbrewer.RdYlBu[min(max(len(timepoints), 3), 11)]
    ]
...
def leaf_style(node, seqmeta, tp_colors, highlight_node=None):
    ...
    duplicities = [
        int(n) if n else 0
        for n in seqmeta.get(
            "cluster_timepoint_multiplicities", seqmeta["timepoint_multiplicities"]
        ).split(":")
    ]
    multiplicity = int(seqmeta.get("cluster_multiplicity", seqmeta["multiplicity"]))
    if len(timepoints) != len(duplicities):
        raise Exception(
            "number of timepoints: {} doesn't match number of multiplicities: {}".format(
                len(timepoints), len(duplicities)
            )
        )
    percents = [d * 100 / multiplicity for d in duplicities]
    colors = [tp_colors[t] for t in timepoints]
    pie_node = ete3.PieChartFace(
        percents,
...
def get_args():
    ...
    parser.add_argument("tree", type=existing_file, help="tree file, in newick format")
    parser.add_argument(
        "--supports", action="store_true", help="tree file, in newick format"
    )
    parser.add_argument(
        "seqmeta",
        type=seqmeta_input,
