...
def parse_seqdict(fh, mode="dnaml"):
    ...
    for line in fh:
        ...
        if m:
            seqs[m.group("id")] += without_ambiguous_nucleotides(m.group("seq").replace(" ", ""))
        elif line.rstrip() == "":
            continue
        else:
            break
...
def parse_outfile(outfile, seqmeta, inferred_naive_name, seqname_mapping=None):
    ...
    with open(outfile, "rU") as fh:
        for sect in sections(fh):
            if sect == "parents":
                parents = {
                    full_name(parent): (full_name(child), dist)
                    for parent, child, dist in iter_edges(fh)
                }
            elif sect[0] == "sequences":
                d = parse_seqdict(fh, sect[1])
                sequences = [
                    SeqRecord(Seq(seq), id=full_name(seq_id), description="")
...
def build_tree(sequences, parents):
    ...
    def mknode(r):
        n = ete3.Tree(
            name=r.id, dist=parents[r.id][1] if r.id in parents else 0, format=1
        )
    ...
    if orphan_nodes != 1:
        raise RuntimeError(
            "The tree is not properly rooted; expected a single root but there are {}.".format(
                orphan_nodes
            )
        )
    return tree
...
def find_node(tree, pattern):
    ...
    if not nodes:
        warn(
            "Cannot find matching node; looking for name matching '{}'".format(pattern)
        )
        return
    else:
        if len(nodes) > 1:
            warn(
                "multiple nodes found; using first one.\nfound: {}".format(
...
def reroot_tree(tree, pattern):
    ...
    if tree != node:
        ...
        if node not in tree.children:
            raise ValueError(
                "Naive node not set as outgroup; Check dnaml/asr run to make sure this is the case"
            )
...
def main():
    ...
    sequences, parents = parse_outfile(
        args.phylip_outfile,
        args.seqmeta,
        args.inferred_naive_name,
        args.seqname_mapping,
    )

    if not sequences or not parents:
        raise RuntimeError(
            "No sequences were available; are you sure this is a dnaml output file?"
        )
    ...
    ancestors_naive_and_seed = ancestors.union(
        set([args.inferred_naive_name, args.seed])
    )
    fname = outbase + ".ancestors_naive_and_seed.fa"
    with open(fname, "w") as fh:
        SeqIO.write(
            [record for record in sequences if record.id in ancestors_naive_and_seed],
            fh,
            "fasta",
        )
