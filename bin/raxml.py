#!/usr/bin/python
# rework this for https://github.com/matsengrp/cft/issues/170
import argparse
import contextlib
import os.path
import shutil
...
def raxml(
    ...
    log_path=None,
    quiet=False,
    executable="raxmlHPC-SSE3",
    model="GTRGAMMA",
    threads=None,
    rapid_bootstrap=None,
    bootstrap_seed=None,
    tmp_prefix=None,
    outgroup=None,
):
    ...
    with temp_dir(prefix="raxml-") as td:
        with sequences_in_format(
            sequences, fmt="phylip-relaxed", prefix=tmp_prefix, dir=td
        ) as seq_file:
            p = joiner(td)
            ...
            if rapid_bootstrap:
                cmd.extend(("-f", "a", "-x", bootstrap_seed, "-N", rapid_bootstrap))
...
def main():
    parser = argparse.ArgumentParser(
        description="""Simple wrapper around
    ...
    parser.add_argument(
        "alignment_file", type=argparse.FileType("r"), help="""Input alignment"""
    )
    parser.add_argument(
        "--input-format",
        default="fasta",
    ...
    parser.add_argument("-o", "--outgroup", help="""Fix output for tree""")

    args = parser.parse_args()
    if not args.executable:
        args.executable = "raxmlHPC-PTHREADS-SSE3" if args.threads else "raxmlHPC-SSE3"
