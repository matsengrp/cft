...
def get_args():
    parser = argparse.ArgumentParser(
        description="Swiss army knife for cft datasets. Filter and merge cft dataset files."
    )
    parser.add_argument(
        "-i",
        "--input",
        type=set_input_of(dataset_infile),
        help="':'-separated set of yaml or json dataset input files",
    )
    parser.add_argument(
        "-o",
        "--output",
        help="dataset output yaml or json file (defaults to stdout as yaml)",
    )
    parser.add_argument("--id", help="output dataset id")
    parser.add_argument(
        "--samples", type=set_input, help="':'-separated set of sample ids to filter to"
    )
    parser.add_argument(
        "--subjects",
        type=set_input,
        help="':'-separated set of subject ids to filter to",
    )
    parser.add_argument(
        "--seeds", type=set_input, help="':'-separated set of seed ids to filter to"
    )
    parser.add_argument(
        "--unseeded", action="store_true", help="include unseeded partitions"
    )
    parser.add_argument(
        "--seeded", action="store_true", help="include seeded partitions"
    )
    parser.add_argument("--sample-prefix", help="prefix all sample ids with this value")
    args = parser.parse_args()
    # apply defaults
    args.id = args.id or (
        args.output.replace("/", "-") if args.output else str(uuid.uuid4())
    )
    return args
...
def apply_unseeded_filter(args, sample):
    if not args.unseeded and (args.seeded or args.seeds):
        return subset_by_keys(
            sample,
            set(sample.keys()).difference(
                ["partition-file", "cluster-annotation-file", "other-partitions"]
            ),
        )
    else:
        return sample
...
def remove_empty_samples(args, samples):
    return {
        ...
        if sample.get("seeds")
        or (sample.get("partition-file") and sample.get("cluster-annotation-file"))
        or sample.get("other-partitions")
    }


def prefix_sample_ids(args, samples):
    return (
        map_keys(lambda k: args.sample_prefix + "-" + k, samples)
