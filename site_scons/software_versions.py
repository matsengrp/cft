...
def software_info(prog):
    ...
    return {
        "cft.software:name": prog,
        "cft.software:version": version_command()
        if callable(version_command)
        else (call_check_command(version_command) if version_command else None),
        "cft.software:which": subprocess.check_output(["which", prog])
        if not callable(version_command)
        else None,
    }
